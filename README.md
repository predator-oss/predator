...
[![Docker Pulls](https://badgen.net/docker/pulls/ghcr.io/predator-oss/predator)](https://hub.docker.com/r/ghcr.io/predator-oss/predator)

<h1 align="center">
<img src="https://predator-oss.github.io/predator/images/predator-1764x640.png" data-canonical-src="https://predator-oss.github.io/predator/images/predator-1764x640.png"/>
</h1>
...
[**Documentation**](https://predator-oss.github.io/predator/) &nbsp; |
&nbsp; [**Installation**](https://predator-oss.github.io/predator/installation.html) &nbsp; |
&nbsp; [**API Reference**](https://predator-oss.github.io/predator/indexapiref.html) | 
...
Create Chaos experiments by submitting Kubernetes JSON formatted CRD, check out our [API documentation](https://predator-oss.github.io/predator/indexapiref.html#operation/create-a-job) for more details.
...
To see the progress of `v1.5.0` [click here](https://github.com/predator-oss/predator/issues?q=is%3Aissue+is%3Aopen+label%3A1.5.0)
...
#### Jobs API
2. Functional tests support will break the `POST /jobs` API. A new parameter `type` to the create job body will be added and will require either a value of `load_test` or `functional_test`, check out our [API documentation](https://predator-oss.github.io/predator/indexapiref.html#operation/create-a-job) for more details.
3. New Webhooks API will break the current webhooks feature implemented in <= v1.4. `POST /jobs` API will now include in the body `webhook_id` field instead of `webhook_url`.
...
![](https://predator-oss.github.io/predator/images/predator-overview.png)
...
### Kubernetes
Predator is designed to seamlessly deploy into your Kubernetes cluster. Install Predator from the [predator-oss helm repository](https://github.com/predator-oss/helm-charts):
...
```docker run -d -e JOB_PLATFORM=DOCKER -e INTERNAL_ADDRESS=http://$MACHINE_IP:80/v1 -p 80:80 --name predator -v /var/run/docker.sock:/var/run/docker.sock ghcr.io/predator-oss/predator```

With persisted storage:

```docker run -d -e SQLITE_STORAGE=db/predator -e JOB_PLATFORM=DOCKER -e INTERNAL_ADDRESS=http://$MACHINE_IP:80/v1 -p 80:80 --name predator -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/predator:/usr/db ghcr.io/predator-oss/predator```
...
if PREDATOR_DOCS_URL was not specified, the default value is `https://predator-oss.github.io/predator`.
...
Please read [CONTRIBUTING.md](https://github.com/predator-oss/predator/blob/master/CONTRIBUTING.md) for details on our code of conduct, and the process for submitting pull requests to us.

## Versioning

We use [SemVer](http://semver.org/) for versioning. For a complete list of Docker images of this project please visit our [docker hub repository](https://hub.docker.com/r/ghcr.io/predator-oss/predator/tags).

Predator tagged images are maintained in two ways:

1. MAJOR.MINOR (for example ghcr.io/predator-oss/predator:1.8)
2. MAJOR.MINOR.PATCH (for example ghcr.io/predator-oss/predator:1.8.4)
